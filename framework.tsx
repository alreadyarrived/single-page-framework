...
const AppRouter = {
  ...
  add(path: string, config: RouteConfig | React.ComponentType<any>) {
...
interface TestInterface {
  cases: Map<string, () => void | Promise<void>>;
  add(n: string, f: () => void | Promise<void>): void;
  run(): Promise<void>;
}

const Test: TestInterface = {
  cases: new Map(),
  add(n: string, f: () => void | Promise<void>) {
    this.cases.set(n, f);
  },
  async run() {
...
class CoreFramework {
  ...
  get test() {
    return this.getService<TestInterface>(SERVICE_TOKENS.TEST);
  }
...
// Framework Context
const FrameworkContext = createContext<FrameworkImpl | null>(null);
...
class FrameworkImpl extends CoreFramework {
  ...
  private readonly _testFramework: TestInterface = this.getService<TestInterface>(SERVICE_TOKENS.TEST);
  ...
  // Test Registration
  registerTest(n: string, f: () => void | Promise<void>) {
  ...
  route(path: string, config: RouteConfig | React.ComponentType<any>) {
    if (typeof config === 'function') {
  ...
  routeGroup(layout: React.ComponentType<any>) {
    return (path: string, component: React.ComponentType<any>) => {
      ...
      useEffect(() => {
        const loadRoute = async () => {
          try {
            setLoading(true);
            const path = location;
            const resolved = await framework.router.resolve(path);

            if (resolved) {
              const { component: RouteComponent, context, layout: LayoutComponent } = resolved;

              const content = (
                <RouteComponent context={context} {...value} />
              );

              setRouteComponent(LayoutComponent ? (
...
export function FrameworkProvider({
  ...
  const [framework] = useState(() => new FrameworkImpl(config));
